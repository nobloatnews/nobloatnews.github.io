...
for cmd in ffmpeg ffprobe yt-dlp espeak-ng slider gen.sh fzf magick; do
...
prueba=0
thumb_ancho=320   # ancho/alto máximo de los _thumb.jpg que se suben a Archive
...
echo "Genero thumbnails de las imágenes..."
if (($# > 2)); then
  (
    cd "$3"
    for img in *.jpg; do
      [ -e "$img" ] || continue
      case "$img" in
        *_thumb.jpg) continue ;;   # no le genero thumbnail a un thumbnail
      esac
      thumb="${img%.*}_thumb.jpg"
      [ -f "$thumb" ] && continue  # ya existe, no lo piso
      magick "$img" -resize "${thumb_ancho}x${thumb_ancho}" -quality 85 "$thumb"
    done
  )
fi
